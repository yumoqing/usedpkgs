...
var bufferview = $.extend({}, $.fn.datagrid.defaults.view, {
	...
	onBeforeRender: function(target){
		...
		function scrolling(){
			...
			/* 解决datagrid含toolbar时只显示第一页的问题 begin */
			var tb_height = 0;
			if (opts.toolbar){
				tb_height = $(opts.toolbar).height();
			}
			/* 解决datagrid含toolbar时只显示第一页的问题 begin */
			...
			改未下面的判断，解决有toolbar时只显示第一页数据的bug
			*/
			} else if (dc.body2.scrollTop() >= getDataHeight() - dc.body2.height() - tb_height){
				this.getRows.call(this, target, function(rows){
					this.rows = rows;
					this.populate.call(this, target);
				});
